...
.controller("laboratoryController", ["$scope", "$http", "$rootScope", "$window", "printService", 'FileSaver', 'Blob', '$location', '$interval', 'dsp', '$timeout', function ($scope, $http, $rootScope, $window, printService, FileSaver, Blob, $location, $interval, dsp, $timeout) {
  ...
  $scope.express_server = {
    ...
    link: "http://cassandra.vn:8081",
  };

  var socket = io.connect($scope.diag_server.link);
  ...
  $scope.updateChartData = function(index, value) {
    if (index < $scope.window_leng - 2) {
      ...
      // socket.emit("get_sample_ecg_data", {
      //   number_of_samples: $scope.window_length,
  ...
  socket.on("sample_ecg_sent_from_server", function(data) {
    ...
    if (data.length > 0) {
      ...
      if (data.length > 0) {
        for (var result_ind = 0; result_ind < data.length; result_ind++) {
          ecg_bin.push(data[result_ind]);
        };
      };
    };
  });
  ...
  $scope.save_this_record = function() {
    $scope.new_record = {
      name: "Normal and healthy ECG",
      date: new Date(),
      data_link: "http://localhost:2000/bin/saved-records/Normal_and_healthy_ECG.txt",
      description: "My ECG while resting with some light coffee and music",
      clinical_symptoms: {
        chest_pain: false,
    ...
    $http({
      method: "POST",
      url: $scope.diag_server.link + "/save_this_record",
    ...
    $scope.new_record.record_data.data = [];
